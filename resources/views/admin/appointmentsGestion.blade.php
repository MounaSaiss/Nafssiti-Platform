@extends('admin.layouts.app')

@section('title', 'Gestion des Rendez-vous | NAFSSITI PRO')

@section('page_title', 'Gestion des Appointments')

@section('content')
    <div class="flex flex-wrap gap-4 mb-6">
        <div class="relative flex-1 min-w-[300px]">
            <i class="fas fa-search absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 text-xs"></i>
            <input type="text" placeholder="Rechercher par patient ou psychologue..."
                class="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded text-sm outline-none focus:ring-1 focus:ring-nafssiti-primary shadow-sm transition">
        </div>
        <select
            class="bg-white border border-slate-200 px-4 py-2.5 rounded text-xs font-bold uppercase text-slate-600 outline-none cursor-pointer">
            <option>Tous les statuts</option>
            <option>En attente</option>
            <option>Confirmé</option>
            <option>Refusé</option>
        </select>
    </div>

    <div class="bg-white border border-slate-200 shadow-sm rounded-sm overflow-hidden">
        <table class="w-full text-left text-xs border-collapse">
            <thead>
                <tr class="bg-slate-50 text-slate-500 uppercase tracking-widest border-b border-slate-200">
                    <th class="px-6 py-4 font-bold">ID / Date de Création</th>
                    <th class="px-6 py-4 font-bold">Patient</th>
                    <th class="px-6 py-4 font-bold">Psychologue (ID)</th>
                    <th class="px-6 py-4 font-bold">Date & Heure RDV</th>
                    <th class="px-6 py-4 font-bold text-center">Statut</th>
                    <th class="px-6 py-4 font-bold text-right">Actions Admin</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-slate-100">

                <tr class="hover:bg-slate-50 transition-colors">
                    <td class="px-6 py-4">
                        <div class="flex flex-col">
                            <span class="font-mono text-slate-400 text-[10px]">#APP-9982</span>
                            <span class="text-[9px] text-slate-400">Créé le : 09/03/2026</span>
                        </div>
                    </td>
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-2">
                            <div
                                class="w-7 h-7 bg-slate-100 rounded-full flex items-center justify-center font-bold text-slate-500">
                                K</div>
                            <span class="font-bold text-slate-900">Karim Idrissi</span>
                        </div>
                    </td>
                    <td class="px-6 py-4">
                        <div class="flex flex-col">
                            <span class="font-bold text-nafssiti-primary uppercase">Dr. Sara Mansouri</span>
                            <span class="text-[10px] font-mono text-slate-400">Psy ID: 452</span>
                        </div>
                    </td>
                    <td class="px-6 py-4">
                        <div class="flex flex-col">
                            <span class="font-bold text-slate-700">15 Mars 2026</span>
                            <span class="text-slate-500 italic text-[10px]">14:30 (GMT+1)</span>
                        </div>
                    </td>
                    <td class="px-6 py-4 text-center">
                        <span
                            class="inline-flex items-center px-2 py-0.5 rounded text-[9px] font-bold uppercase bg-amber-50 text-amber-500 border border-amber-100">
                            En attente
                        </span>
                    </td>
                    <td class="px-6 py-4">
                        <div class="flex justify-end gap-2">
                            <button
                                class="px-3 py-1.5 bg-nafssiti-secondary text-white rounded-sm font-bold uppercase text-[9px] hover:bg-green-600 transition shadow-sm">
                                <i class="fas fa-check mr-1"></i> Accepter
                            </button>
                            <button
                                class="px-3 py-1.5 bg-nafssiti-red text-white rounded-sm font-bold uppercase text-[9px] hover:bg-red-600 transition shadow-sm">
                                <i class="fas fa-times mr-1"></i> Refuser
                            </button>
                        </div>
                    </td>
                </tr>

                <tr class="hover:bg-slate-50 transition-colors bg-slate-50/30">
                    <td class="px-6 py-4">
                        <div class="flex flex-col">
                            <span class="font-mono text-slate-400 text-[10px]">#APP-9975</span>
                            <span class="text-[9px] text-slate-400">Créé le : 08/03/2026</span>
                        </div>
                    </td>
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-2">
                            <div
                                class="w-7 h-7 bg-slate-100 rounded-full flex items-center justify-center font-bold text-slate-500">
                                M</div>
                            <span class="font-bold text-slate-900">Mouna Tazi</span>
                        </div>
                    </td>
                    <td class="px-6 py-4">
                        <div class="flex flex-col">
                            <span class="font-bold text-nafssiti-primary uppercase">Dr. Amine Bennani</span>
                            <span class="text-[10px] font-mono text-slate-400">Psy ID: 128</span>
                        </div>
                    </td>
                    <td class="px-6 py-4">
                        <div class="flex flex-col">
                            <span class="font-bold text-slate-700">12 Mars 2026</span>
                            <span class="text-slate-500 italic text-[10px]">10:00 (GMT+1)</span>
                        </div>
                    </td>
                    <td class="px-6 py-4 text-center">
                        <span
                            class="inline-flex items-center px-2 py-0.5 rounded text-[9px] font-bold uppercase bg-green-50 text-nafssiti-secondary border border-green-100">
                            Confirmé
                        </span>
                    </td>
                    <td class="px-6 py-4">
                        <div class="flex justify-end gap-2">
                            <button
                                class="px-3 py-1.5 bg-slate-100 text-slate-400 cursor-not-allowed rounded-sm font-bold uppercase text-[9px]">
                                <i class="fas fa-check mr-1"></i> Accepté
                            </button>
                            <button
                                class="px-3 py-1.5 border border-nafssiti-red text-nafssiti-red hover:bg-nafssiti-red hover:text-white rounded-sm font-bold uppercase text-[9px] transition">
                                <i class="fas fa-ban mr-1"></i> Annuler
                            </button>
                        </div>
                    </td>
                </tr>

            </tbody>
        </table>
    </div>
@endsection
