...
@section('content')
                ...
                @if ($nextAppointment)
                    <div class="flex flex-wrap items-center gap-4 mt-1">
                        <p class="text-lg font-bold text-slate-800">
                            {{ \Carbon\Carbon::parse($nextAppointment->appointmentDate)->translatedFormat('d M Y') }} à
                            {{ \Carbon\Carbon::parse($nextAppointment->appointmentTime)->format('H:i') }}
                        </p>
                        <span
                            class="text-xs px-2 py-0.5 bg-slate-100 text-slate-600 rounded font-medium border border-slate-200">
                            <i class="fas fa-video mr-1"></i>
                            {{ $nextAppointment->psychologist->user->name ?? 'Psychologue' }}
                        </span>
                        <span
                            class="text-[9px] px-2 py-0.5 bg-green-50 text-green-600 rounded-full font-bold border border-green-100 uppercase tracking-tighter">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <a href="{{ route('psychologue.allPsychologues') }}"
                ...
                <div class="flex items-center gap-4">
                    <div
                        class="w-10 h-10 bg-slate-50 group-hover:bg-teal-50 text-slate-400 group-hover:text-nafssiti-primary flex items-center justify-center rounded-sm transition">
                        <i class="fas fa-search"></i>
                    </div>
                    <div>
                        <p class="font-bold text-slate-900 group-hover:text-nafssiti-primary transition">Trouver un
                            psychologue</p>
            ...
            <a href="{{ route('patient.rendezVous') }}"
                ...
                <div class="flex items-center gap-4">
                    <div
                        class="w-10 h-10 bg-slate-50 group-hover:bg-green-50 text-slate-400 group-hover:text-nafssiti-secondary flex items-center justify-center rounded-sm transition">
                        <i class="fas fa-calendar-alt"></i>
                    </div>
                    <div>
                        <p class="font-bold text-slate-900 group-hover:text-nafssiti-secondary transition">Mes rendez-vous
                        </p>
    ...
    <div
        class="bg-nafssiti-dark text-white p-8 rounded-sm shadow-lg flex flex-col md:flex-row items-center justify-between gap-6">
        <div class="max-w-md">
            <h4 class="text-lg font-bold mb-2 uppercase tracking-tighter">Votre bien-être est notre priorité.</h4>
            <p class="text-slate-400 text-sm">N'oubliez pas que chaque étape, aussi petite soit-elle, est une victoire vers
