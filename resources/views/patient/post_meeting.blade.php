...
@section('content')
    <div class="max-w-2xl mx-auto mt-6">
        ...
        @else
            @php $currentFollowRequest = $followRequests->where('psychologist_id', $appointment->psychologist_id)->first(); @endphp
            {{-- CARTE PRINCIPALE --}}
            <div class="bg-white rounded-sm shadow-sm border border-slate-200 overflow-hidden">
                ...
                @if ($appointment->consultation_status === 'pending')
                    ...
                    <div class="bg-blue-50 border-b border-blue-100 p-6 text-center">
                        ...
                        <div class="mt-6">
                            @if ($currentFollowRequest && $currentFollowRequest->status === 'accepted')
                ...
                @else
                    ...
                    <div id="decision-section">
                        ...
                        <div class="p-6">
                            <div id="action-buttons-grid" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                @if (!$currentFollowRequest)
                                    <div class="text-center mb-6 col-span-2">
                                        <h3 class="text-base font-bold text-slate-800">
                                            Êtes-vous satisfait(e) pour continuer avec {{ $appointment->psychologist->user->name }}
                                            ou non ?
                                        </h3>
                                    </div>
                                    ...
                                    <form action="{{ route('patient.follow_request.store', $appointment) }}" method="POST">
                                        ...
                                        <button type="submit"
                                            ...
                                            <div>
                                                ...
                                                <p class="text-xs text-slate-500 mt-0.5">Je souhaite continuer les séances.
                                                </p>
                                            </div>
                                        </button>
                                    </form>
                                    ...
                                    <button type="button" onclick="handleNotSatisfied()"
                                        ...
                                        <div>
                                            <h4
                                                class="text-sm font-bold text-slate-900 group-hover:text-red-500 transition">
                                                Non satisfait(e)</h4>
                                ...
                                @if ($followRequests->isNotEmpty())
                                    <div class="md:col-span-2 mt-4">
                                        <h4 class="text-sm font-bold text-slate-800 mb-3 border-b pb-2">Vos demandes de
                                            suivi</h4>
                                            ...
                                            @foreach ($followRequests as $request)
                                                @php
                                                    $statusClasses = [
                                                        ...
                                                        'text' => 'text-green-700',
                                                        'title_text' =>
                                                            'Demande envoyée à Dr. ' .
                                                            ($request->psychologist->user->name ?? 'Praticien'),
                                                    ];
                                                    ...
                                                    if ($request->status === 'accepted') {
                                                        ...
                                                        $statusClasses = [
                                                            ...
                                                            'text' => 'text-red-700',
                                                            'title_text' =>
                                                                'Demande refusée par Dr. ' .
                                                                ($request->psychologist->user->name ?? 'Praticien'),
                                                        ];
                                                    }
                                                @endphp
                                                <div
                                                    class="border rounded-sm p-4 flex flex-col sm:flex-row items-start sm:items-center gap-4 {{ $statusClasses['wrapper'] }}">
                                                    <div
                                                    ...
                                                    <div class="flex-1">
                                                        ...
                                                        <div class="text-xs mt-0.5 {{ $statusClasses['text'] }}">
                                                            @if ($request->status === 'pending')
                                                                <p>Votre demande de suivi est en attente d'acceptation.</p>
                                                            @elseif($request->status === 'accepted')
                                                                <p>Votre demande de suivi a été <strong>acceptée</strong>.
                                                                </p>
                                                                <div class="mt-2">
                                                                    <a href="{{ route('patient.shared_room.index', $request->psychologist_id) }}"
                                                                        class="inline-flex items-center gap-2 bg-nafssiti-primary hover:bg-teal-500 text-white px-4 py-2 rounded-sm font-bold shadow-sm transition-all transform hover:-translate-y-0.5">
                                                                        <i class="fas fa-desktop"></i> Accéder à mon Espace
                                                                        de Suivi
                                                                    </a>
                                                                </div>
                                                            @elseif($request->status === 'rejected')
                                                                <p>Le praticien ne peut malheureusement pas démarrer de
                                                                    suivi avec de nouveaux patients actuellement.</p>
                                                            @endif
                                                        </div>
                                                    </div>
                                                </div>
                        ...
                        <div id="msg-not-satisfied" class="hidden p-6 border-t border-slate-100">
                            <div
                                class="bg-red-50 border border-red-200 rounded-sm p-4 flex items-start flex-col sm:flex-row gap-4">
                                <div
                                    class="w-10 h-10 bg-red-100 text-red-600 flex items-center justify-center rounded-sm shrink-0">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-red-700 mt-1 leading-relaxed">
                                        Il est tout à fait normal de ne pas trouver le praticien idéal du premier coup.
                                        Votre
